...
class JFormFieldJaDepend extends JFormField
{
    ...
    protected function getProfile()
    {
        ...
        $path = JPATH_SITE . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR . $template . DIRECTORY_SEPARATOR . 'html' . DIRECTORY_SEPARATOR . $module . 'profiles';
        ...
        $xmlparams = JPATH_SITE . DIRECTORY_SEPARATOR . 'modules' . DIRECTORY_SEPARATOR . $module . DIRECTORY_SEPARATOR . 'admin' . DIRECTORY_SEPARATOR . 'config.xml';
        if (file_exists($xmlparams)) {
            /* For General Form */
            $jaform = JForm::getInstance('jform', $xmlparams, array('control' => 'jaform'));

			$profileHTML = JHTML::_('select.genericlist', $profiles, '' . $this->name, 'onchange="japrofile.changeProfile(this.value)"', 'id', 'title', $this->value);
