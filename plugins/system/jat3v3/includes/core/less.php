...
class T3V3Less extends lessc
{
	...
	function compileCss ($path, $topath = '') {
		...
		// add vars
		$this->setImportDir (array(dirname($realpath), T3V3_TEMPLATE_PATH.'/assets/less/'));
	...
	public static function buildVars ($theme=null) {
		...
		$path = T3V3_TEMPLATE_PATH.'/assets/less/vars.less';
		...
		if (count($matches[0])) {
			foreach ($matches[1] as $url) {
				$path = T3V3::cleanPath(T3V3_TEMPLATE_PATH.'/assets/less/'.$url);
				if(file_exists($path) && $last_modified < filemtime ($path)){
					$last_modified = filemtime ($path);
				}
			}
		}
		...
		if ($theme) {
			...
			$path = T3V3_TEMPLATE_PATH.'/assets/less/themes/'.$theme.'/variables.less';
			if (is_file ($path)) {
			...
			$path = T3V3_TEMPLATE_PATH.'/assets/less/themes/'.$theme.'/variables-custom.less';
			if (is_file ($path)) {
				if ($last_modified < filemtime ($path)) $last_modified = filemtime ($path);
				// append theme file into vars
	...
	public static function addStylesheet ($lesspath) {
		...
		if (defined ('T3V3_THEMER')) {
			...
			$doc->addScript (T3V3_URL.'/assets/js/less-1.3.0.js');
		} else {
	...
	public static function compileAll ($theme = null) {
		...
		$files = array ();
		$lesspath = 'templates/'.T3V3_TEMPLATE.'/assets/less/';
		$csspath = 'templates/'.T3V3_TEMPLATE.'/assets/css/';
