                    ...
                    html = function () {
						...
						if (!inst.inline) {
							...
							if (inst.options.showCancelButton) {
								html += '<button class="ui-colorpicker-cancel btn">' + inst._getRegional('cancel') + '</button>';
							}
							html += '<button class="ui-colorpicker-ok btn btn-primary">' + inst._getRegional('ok') + '</button>';
							html += '</div>';
						}
	...
	$.widget("vanderlee.colorpicker", {
			...
			if (this.element[0].nodeName.toLowerCase() === 'input') {
				...
				if (that.options.showOn === 'button' || that.options.showOn === 'both') {
					...
					if (that.options.buttonImageOnly && that.image) {
						...
						that.button = $('<button type="button" class="colorpicker"></button>').html(that.image || that.options.buttonText).button();
		...
		open: function() {
			...
			if (!that.opened) {
				...
				x		= offset.left+ that.element.outerWidth();
		...
		close: function (cancel) {
			...
			that._effectHide(that.dialog, function () {
				...
				that.opened		= false;
				that._callback('close', true, cancel);
			});
		...
		_callback: function (callback, spaces, iscancel) {
			var that = this,
				data,
				lab;
